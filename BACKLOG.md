...
| **Contextual range, reach and travel bases** | **Specified but absent** | Every authored Range, execution duration and travel speed is a literal, and they are **provisional rules nobody has approved** — `punch` 1.5 m, `parry` 2 m, `fire-blast` 15 m at 30 m/s, one-second strikes. Range must stay data (it decides *eligibility*, which adjudication does not answer), but the figures want contextual bases instead: reach derived from the body doing the reaching, a Reaction's range from the action it answers, a projected range from declared power. `DistanceInterval` has no such base today. Until then the literals stand, flagged in `capabilities/skills.ts`. |
