...
| **Combat check design for authored Skills** | **Specified but absent** | `punch`, `parry` and `fire-blast` deliberately declare `check: {kind:"adjudicated"}` with guided-narrative outcomes rather than naming an attack scope nobody has designed, and `fire-blast` prices no Aura pending a request-time cost profile keyed to declared power. No authored Skill declares Mastery changes for the same reason. Deciding these is game design, not a code gap — the contracts are ready to receive it. |
