...
| Mechanic | State | Detail |
|---|---|---|
| **Runtime ownership and transition protocol** | Complete | Ownership matrix, `TransitionResult`, routing-only request base with `QuantitativeRequest` for amounts, owner refs (domain + id) supporting multi-character operations with per-owner calculation context, discardable transaction draft keyed by owner, cumulative same-owner costs, simultaneous batch settlement by complete owner, phase, missing-state and complete-outcome validation, generic coordinator. `runtime/`. Exported. Dependency-tested to contain no gameplay. |
| **Aura cost-handler reference** | Complete | `aura/runtime.ts`. A domain owning a spendable resource others need. Stateless in both senses: it holds neither a pool nor any one character's context, resolving each owner's from the request. |
...
| **Skill execution and Accuracy modification** | **Partially implemented** | `checks/` resolves d20 checks including opposed and fixed forms, and `Accuracy` is a Derived Attribute. Every Skill now carries a validated executable application contract and live availability resolves (`capabilities/applications.ts`, `application-resolution.ts`), but nothing *executes* one: no check is rolled, no cost committed, no outcome applied. `ImprovisedSkillAttempt` is a type with no resolution. Most of `checks/` is also unexported (below). **Constraint for that ticket:** `buildSkillActionProfile()` is lossy by design — `ActionProfile` holds one check scope, while the application holds the opponent's scope, `tiesFavor`, a fixed check's `tiePolicy`, the exertion/Aura costs and the outcome branches. Preparation must receive the `EffectiveSkillApplication` alongside the profile; taking the profile alone silently resolves every opposed Skill as unopposed and charges nothing. |
| **Combat check design for authored Skills** | **Specified but absent** | `punch`, `parry` and `fire-blast` deliberately declare `check: {kind:"adjudicated"}` with guided-narrative outcomes rather than naming an attack scope nobody has designed. No authored Skill declares Mastery changes for the same reason. Deciding these is game design, not a code gap — the contracts are ready to receive it. |
| **Request-derived Aura pricing** | **Partially implemented** | `SkillAuraCostProfile` distinguishes `none` / `fixed` / `request-derived`, so "burns no Aura" is no longer the same data as "cannot be priced until the character says how hard they are pushing" — which mattered because Aura expenditure charges an omitted figure as **zero**. `fire-blast` declares `{kind:"request-derived", profileId:"aura.declared-power"}` and `projectSkillAuraCost()` returns a variant carrying no Aura figure, so preparation must branch rather than charge nothing. **Nothing resolves the profile yet**: no construction rule exists for `aura.declared-power`, and the execution ticket owes one. |
| **Context-value construction rules** | **Partially implemented** | The *declaration* is done: Range, execution duration and travel are `SkillApplicationValue<T>`, every authored Skill names the profile that must supply its figures, and `buildSkillActionProfile()` refuses to produce a profile until each is supplied, matches the authored `profileId`, and passes its neutral-domain validator. **No construction rule exists for any profile** — `body.reach`, `combat.action-duration`, `combat.reaction-duration`, `reaction.trigger-range`, `task.lockpicking-range`, `task.lockpicking-duration`, `aura.declared-power.range`, `aura.declared-power.travel`. Body-reach calculation, lock-complexity timing, Reaction inheritance and declared-power projection are execution work. Consequence today: `punch`, `parry`, `defensive-stance`, `pick-lock` and `fire-blast` resolve and validate but **cannot be projected** without a caller supplying the values. Nothing unresolved may move into `spatial/` to fix this — `DistanceInterval` stays resolved metres, enforced by `architecture.test.ts`. |
