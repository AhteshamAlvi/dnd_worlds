...
export const ENGINE_DECISIONS = {
    ...
    "combat.reactions.queued-in-initiative-order": {
        ...
        rationale:
            "Initiative order is the point. Something has to decide who answers first, and using the order a host listed its targets in would let presentation decide a mechanical outcome; the Round already has an authority for 'who goes first' and it costs nothing to reuse. Ending the Turn exactly once matters because the Turn can only be ended once and later openings have nothing left to close — carrying that as queue state is cheaper than inferring it. Eligibility is checked when a Reaction OPENS rather than when its gate passed, because the shared Round pool is the only Reaction limit and a combatant can be emptied between the two. There is still no per-Round Reaction count: a combatant may answer later threats in the same Round while Actions remain. AMENDED by combat.reactions.queue-is-a-state-machine: the first version was two mutable arrays and a boolean, which let a caller reach states no sequence of play could.",
    },
    "combat.actions.zero-cost-is-legal": {
        id: "combat.actions.zero-cost-is-legal",
        question:
            "isValidActionCost() required a positive whole number, so a GM who waived an Action cost produced an authorization Combat then refused. StructuredActionCost has always defined zero as meaningful.",
        chosen:
            "An Action cost is a whole NON-NEGATIVE number. A zero-cost Action schedules and resolves, spends nothing from the Round pool, and adds nothing to the state's spent count. Inaction and Hesitation stay fixed at one Action. canAffordRoundActionCost() treats zero as affordable by anybody, including a combatant with an empty pool.",
        rationale:
            "Refusing zero made the GM's waiver unusable at the one place it mattered, which is the same defect as the scheduler re-deciding timing: a ruling that cannot survive to the point of use is decoration. The important thing this is NOT is a Bonus Action. A free Action consumes nothing and GRANTS nothing — it does not extend what the Turn may still do, it is not limited to one, and nothing authorizes it beyond the ordinary adjudication that waived the cost. Bonus Actions remain removed and undesigned, and a test asserts the remaining Turn allowance is unchanged specifically so this cannot be read as an accidental implementation of them. Inaction and Hesitation keep their cost because deliberately doing nothing, and failing to decide in time, are both supposed to cost you the Action.",
    },
    "combat.reactions.queue-is-a-state-machine": {
        id: "combat.reactions.queue-is-a-state-machine",
        question:
            "The first multi-target Reaction queue was a record of two mutable arrays and a boolean, coordinated by callers calling the helpers in the right order.",
        chosen:
    ...
    "actions.authorization.validated-data-not-a-token": {
        id: "actions.authorization.validated-data-not-a-token",
        question:
            "The scheduling authorization was described as evidence that preparation and adjudication had run. It is a plain readonly TypeScript object, so a caller can build one by hand or edit one they were given.",
        chosen:
    ...
    "spatial.validation.not-throwing-is-not-correct": {
        ...
        rationale:
            "Not throwing is a much weaker property than being right, and testing for it produced exactly the failure it deserved: a guard that stopped the crash and left the wrong diagnostic, and a predicate that stopped reading null and kept lying about the type it narrows to. `position is MetricPosition` is a claim about a spatial context AND three finite coordinates, so accepting the tag alone made every caller's cast unsound — and a first fix that checked only the coordinates was the same mistake one field further in. The context is as load-bearing as the numbers: three coordinates with no space attached describe a point in no particular place, and comparing them against another position is the silent wrong answer the spatial domain exists to refuse. A predicate that narrows to a type must verify every field of it. Refusing to compute while issues stand is the general form: arithmetic on a value already known to be invalid produces a number nobody should read, and NaN in a trace is not JSON — a caller serializing a failure to show a GM gets null where a number should be. A failure trace has to survive the trip as much as a successful one.",
    },
    "capabilities.mastery.optional-and-definition-specific": {
        id: "capabilities.mastery.optional-and-definition-specific",
        question:
            "Every Skill and Technique carried a mandatory Mastery track, possession was inferred from `mastery > 0`, and the requirement context recorded both facts in one id -> rank map. That makes a capability with no ranks unrepresentable: at 0 the character does not have it, and at I they hold a rank nobody can advance and content can then require.",
        chosen:
            "The track is an optional `mastery` field on the definition. A stored rank means I when a track exists and nothing when it does not; a resolved capability carries `supportsMastery` and a `mastery` of a rank OR null, and possession is presence in the resolved record rather than any comparison. Requirements read two separate context fields — `skillIds`/`techniqueIds` for presence, `skillMastery`/`techniqueMastery` for rank — so a trackless capability satisfies hasSkill and can never satisfy a rank requirement. A rank stored against a trackless capability is a validation error, as is content requiring Mastery in one, or requiring a rank past the end of a real track.",
        rationale:
            "Some things a character can do have no depth: you activate the door rune or you do not, and there is no better way to do it. Spelling that as a one-rank track puts a permanently fixed number in front of a player and invites them to spend Growth Points on it. Null rather than 0 is the load-bearing choice — 0 was already taken to mean 'does not have it', which is precisely why the two facts could not be told apart — and splitting the requirement context follows from it, since one map cannot hold a capability that is present and rankless without a sentinel that already means absent. Making the id an entry rather than a rank also removes the ceiling the old projection had: resolveCapabilities takes the authored ENTRIES now, because a record of ranks cannot describe an entry that has none. The unknown-id fallback stays 'masterable', so a typo in a Skill id is reported as an unknown Skill without additionally throwing away the rank the sheet stored beside it.",
    },
    "capabilities.lifecycle.acquisition-is-a-moment": {
        id: "capabilities.lifecycle.acquisition-is-a-moment",
        question:
            "Five different relationships shared one word. A character could have a Skill because they trained it, because an Item was lending it, because their Clan had opened its style to them, because an Injury had permanently awarded it, or because a newer Technique had absorbed the older one it came from. All five resolved identically, and a definition's `requirements` were rechecked on every validation forever — so losing Fire Control deleted Flame Lance from a legal sheet.",
        chosen:
            "Grants carry a mode: granted-while-present (a loan), unlocked-for-acquisition (permission, no access), granted-permanently (an award returned as data for the caller to commit). An omitted mode is a loan, which is what every grant authored before modes meant. A definition's requirements are ACQUISITION requirements, asked once; validation reports an acquired capability whose prerequisites no longer hold as a warning, never a refusal. Resolved capabilities carry availability (available / subsumed / inaccessible), unlockedBy and subsumedBy, and possession is availability rather than presence, because the record now also holds offers nobody has taken up. A definition may additionally declare requiresUnlock, and acquisition eligibility reports both gates separately plus the decision they combine to.",
        rationale:
            "The two gates are orthogonal and neither is the answer alone: meeting a style's Attributes does not make an outsider welcome, and being invited does not confer the Attributes. So permission is a separate field rather than a prerequisite type, and the parts are reported alongside the decision because 'train DEX' and 'get invited' are different things to tell a player. A missing offer is a definite refusal rather than unresolved, since the unlock sources are read off resolved state and their absence means nobody is offering it. Rechecking acquisition requirements forever is the engine answering, every time, a question that was settled when the character learned the thing. Whether they can still USE it is a real question and a different one, asked of the application at execution time. Modes are a field on the Effect rather than three new Effect types because the three do the same thing to the same record and differ only in how long it lasts; splitting them into `unlockSkill` and `awardSkill` would have tripled the grant vocabulary and left every consumer switching over it. Returning awards instead of writing them is what keeps resolveCharacter pure — a permanent grant that reached into the sheet would make resolution unsafe to run while a player edits — and commitCapabilityAwards() is idempotent because the award is reproduced on every resolution for as long as its content applies.",
    },
    "capabilities.subsumption.declared-never-inferred": {
        id: "capabilities.subsumption.declared-never-inferred",
        question:
            "A newer capability replacing an older one could be read off the data already present: a parent Trait, a prerequisite, or a name that reads like a successor. Awakened Sharingan replaces Sharingan; Mangekyo Sharingan does not, and shares both the parent Trait AND the prerequisite with the one that does.",
        chosen:
            "An explicit same-kind `subsumes` list on the definition, and nothing inferred from anything else. parentTraitId stays taxonomy. A subsumed capability keeps its record and its Mastery, satisfies requirements naming it, and stops contributing on its own account; the surviving capability inherits its effects and grants, transitively, and each subsumed capability is inherited exactly once even when two replacements name it. A subsumption cycle leaves everything in it available rather than everything subsumed.",
        rationale:
            "Sharingan and Mangekyo are the whole argument: every signal an inference could use is identical between the pair that coexists and the pair that does not, so anything inferred would be a coin flip that silently retires content nobody meant to retire. Marking the old capability subsumed rather than deleting it is what keeps the acquisition history true and stops an author retiring a Technique from having to hunt down every Skill that named it. Inheriting exactly once matters as soon as two schools claim the same predecessor — the naive reading applies its bonus twice, from two inheritors, and looks correct in both. The cycle behaviour is chosen the same way a body that cannot resolve is: on data already known to be wrong, degrading to \"everything applies\" beats degrading to \"nothing does\", because the second silently blanks a character while the first is merely too generous and is reported.",
    },
    "capabilities.dependencies.reachability-not-cycles": {
        id: "capabilities.dependencies.reachability-not-cycles",
        question:
            "Content is data, so an author can write a Skill requiring a Technique that requires that Skill. The obvious check walks the requirement graph and rejects every cycle.",
        chosen:
            "A least fixed point over what is OBTAINABLE, not a search for loops. Start with nothing; repeatedly add every capability whose requirements are satisfiable by what has already been added; stop when a round adds nothing. Nodes are (capability, rank) pairs, so a Mastery rank that can never be reached is caught separately from a capability that can never be acquired. ACCESS grants and subsumption widen reachability and unlocks deliberately do not; non-capability requirements, `not`, unknown ids, and ranks past a declared maximum are all treated as satisfiable.",
        rationale:
            "A mechanical cycle check rejects working content. `Skill A requires Technique B OR Trait C` with `Technique B requires Skill A` and an independently obtainable Trait C contains a cycle and is perfectly playable — take C, then A, then B — so what matters is reachability, and `any` makes that a boolean question rather than a structural one. Ranks are separate nodes because two Skills whose SECOND ranks require each other are both perfectly acquirable, and a check that stopped at acquisition would pass them. Unknown ids and out-of-range ranks are treated as satisfiable so that catalogs.ts's one accurate message about the typo is not buried under a cascade of derived unreachability about everything downstream of it. `not` is satisfiable because declining to have something is always available, and because a non-monotone clause would make the fixed point depend on iteration order. Unlocks are the one grant mode that does NOT widen reachability, which is the mode's whole meaning: a Clan offering a Skill whose requirements can never be met has issued a real invitation to something still unlearnable, and folding offers into grants here would hide exactly the deadlocks this analysis exists to find. The rule runs both ways, so a capability declared requiresUnlock that nothing obtainable ever offers is caught too, with a message naming which of the two gates is shut.",
    },
    "capabilities.application.usability-is-asked-every-time": {
        id: "capabilities.application.usability-is-asked-every-time",
        question:
            "A Skill said one thing about being used — a flat `timings` list — and nothing about what it cost, what it could be pointed at, how far it reached, how it was decided or what it produced. Acquisition requirements were the only requirements a Skill had, so the only way to express \"Fire Blast needs Fire Control to fire\" was to recheck the acquisition list forever, which deletes a legal Skill from a legal sheet the moment the Trait lapses.",
        chosen:
            "A Skill declares an APPLICATION, and it is REQUIRED: the neutral ActionProfile minus the identity and source a catalog cannot know, plus a mechanical role, its own execution requirements, a physical and Aura cost profile, an automatic/fixed/opposed/adjudicated check, a matching outcome profile, and cumulative Mastery changes. Execution requirements are identified, resolved on every attempt, and never the acquisition list; a held Skill whose execution requirement fails is `requirements-unsatisfied` and stays fully possessed at its rank, and restoring the requirement restores access with nothing written in either direction. Timing moved into the application and nowhere else. Mastery changes are typed operations over a closed field list AND over an outcome branch's own outputs, addressed by (outcome id, output id), so a rank can improve what a Skill DOES rather than only what it costs. Every id inside an outcome — branch, output, consequence — must be present and unique across the application. Deliberate Aura is a REQUIRED discriminated union — none / fixed / request-derived — and each declared Mastery threshold is projected and revalidated in full, including the effective outcome magnitudes. Execution duration and travel are SkillApplicationValue<T>: a fixed constant, or context-derived naming the profile that must supply it. Range is REQUIRED and carries a third variant, none, for a Skill pointed at nothing. Every authored Skill declares its geometry contextually, no construction rule exists yet, and buildSkillActionProfile() refuses to produce a profile until each contextual value is supplied, answers the profileId the Skill named, and passes its own neutral-domain validator. Techniques get none of this.",
        rationale:
            "Possession and usability have different lifetimes, so they are different fields asked at different moments — that is the whole separation, and the flat/nested dual support that a migration usually leaves behind would have handed a scheduler two answers with the weaker one read first. Everything mechanical is COMPOSED rather than redeclared, because a Skill-shaped ActionTiming or TargetSpecification is two structurally identical declarations of a closed vocabulary, which TypeScript accepts assignments between in both directions — the sensory-scope failure, one seam further along, where the disagreement would surface as Combat scheduling something the Skill never permitted. The cost of that composition is one architectural exception, so the two application files are excepted BY NAME from the Character-may-not-import-actions rule and an anti-duplication suite makes composing the only thing they can do with it. Mastery changes are typed operations over a closed field list rather than a deep partial: a deep partial makes \"what does III change\" a structural diff and a mistyped nested key a silent no-op, which is the worst bug a Mastery track can carry because a player has spent Growth Points to reach it. Each declared rank is projected and validated at catalog time for the same reason. The application is required because a Skill is by definition a concrete thing a character does on purpose, so one without a contract is an unfinished definition rather than a kind of capability — and while it was optional the engine gave two answers to one question, resolution refusing the Skill while the requirement helper read its absent list as an empty one and reported it satisfied. The engine still never INVENTS one: minimalSkillApplication() is authoring scaffolding an authoring tool calls to write a real, visible, free-adjudication contract into content, and architecture.test.ts holds that nothing which resolves, validates or authors ever calls it. Mastery had to be able to reach an outcome's magnitude because Mastery is depth: a track that can only make a Skill cheaper or longer-ranged cannot express the same act done harder, which is what ranks are for. And the authored catalog decides no undecided mechanic: Combat has no check layer, so the combat Skills resolve by guided narrative rather than naming an attack scope nobody designed, Fire Blast prices no Aura because a blast's price follows the power the bender declares, and the Mastery demonstration lives in test content — an authored number is a rule the moment it ships, and nobody afterwards can tell a considered value from a placeholder that survived. The Aura cost is a union rather than an optional pair of numbers because Aura expenditure charges an omitted figure as ZERO: \"burns no deliberate Aura\" and \"cannot be priced until the character says how hard they are pushing\" were the same data, and the second silently became the first at the moment of charging — a free Fire Blast that resolves perfectly cleanly. The projection is a union for the same reason, so a caller cannot spread an unpriced cost into a request and have the absence read as nothing owed. Revalidating each rank rather than each authored number is what catches arithmetic that overflows: finite operands can produce Infinity, which is neither a magnitude nor JSON, and no per-number check can see it. The geometric literals are gone rather than flagged. Range must stay data because it decides eligibility rather than success, but the FIGURES were never this catalog\'s to choose: reach belongs to the body doing the reaching, a Reaction\'s range to the action it answers, a blast\'s range and flight to the power declared — so each is context-derived and nothing supplies them yet, which means those Skills validate and cannot be projected. That is the honest state rather than a literal standing in for it. Range is required with an explicit none rather than optional, because an omitted Range and a Range that does not exist are different facts and the optional field could not tell them apart — the permissive reading, no distance requirement at all, is the one it would have handed out for an unfinished Skill. Travel stays optional because it answers a different question: whether anything crosses space. The unresolved half stays entirely in the Skill layer: putting it in DistanceInterval would have been one convenient optional field and would have made the spatial domain a place where a distance is sometimes not a distance, with every consumer of one obliged to handle a case that has no metres in it. Catalog validation is not on the projection path, so the structural pass runs again inside resolveSkillActionValues() and returns before resolving anything: a host that reads a catalog complaint and projects anyway would otherwise receive exactly the permissive answer the complaint was about, and an omitted Range is never translated into a declared none. A supplied value is refused unless it answers the profileId the Skill named and survives the neutral validator, and a value supplied for a field authored as fixed is refused rather than ignored — silently discarding a figure a caller computed is the same class of bug as an omitted Aura cost charged as zero.",
    },
    "equipment.inventory.entries-have-identity": {
        id: "equipment.inventory.entries-have-identity",
        question:
            "A character's inventory was one line per Item — `{ itemId, quantity, equipped }` — with a repeated `itemId` reported as a validation error on the theory that two lines meant a lost quantity. Three ordinary facts were unrepresentable in that shape: a swordsman holding one blade and packing an identical spare, a reference to a PARTICULAR owned object for Shu or an Item use to be applied to, and a trace saying which of two identical gauntlets produced a given bonus.",
        chosen:
            "The inventory holds ENTRIES. Each carries an `entryId` that is unique within the character and unrelated to `itemId`; repeated definition ids are legal and the duplicate-itemId error is deleted rather than renamed. `InventoryItemRef { characterId, entryId }` is the reference type for naming one concrete owned object, and `resolveInventoryItemRef()` answers with a result — invalid-reference, character-mismatch, or unknown-entry — rather than throwing or returning a bare absence. Item Effect sources now carry `{ type: \"item\", id: itemId, instanceId: entryId }`, and `ContributionSourceRef.instanceId` is optional so every instance-less source keeps the identity and key it already had.",
        rationale:
            "The two obvious ways to write the reference are both wrong in ways that cannot be detected at runtime. An array index is not a property of the object: sorting an inventory for display, filtering it for an equipment panel, dropping an earlier entry or round-tripping it through JSON renumbers every line while changing nothing the character owns, so a stored index silently begins naming a different sword and nothing anywhere can tell. A bare `itemId` names every copy at once, so the consumer picks one — by position, which is the first mistake wearing a hat. Identity has to be a field on the entry, and once it is, the duplicate-itemId rule is not merely unnecessary but actively wrong: it was refusing the exact case the model exists to represent. Provenance carries BOTH ids rather than swapping one for the other, because they answer different questions — `hasItem` asks whether the definition is present and would be unsatisfiable against entry ids, while a trace asks which object contributed and cannot say with definition ids alone. Making `instanceId` optional rather than universal is what keeps that additive: a Species, a Clan and a Trait exist once per character by construction, so an instance there would be a second spelling of the definition id. The change surfaced five files that had grown their own `${source.type}:${source.id}` template — in the attribute ladder, in check traces, in modifier selection, in Action capacity — each of which agreed with contributionSourceKey() exactly until a source gained an instance and then began dropping it, producing two contributions that described themselves identically and a trace that disambiguated them with a numeric suffix naming neither. All five now call the one function, and architecture.test.ts refuses a sixth. AMENDED once: the first lookup matched on entryId alone and then narrowed the result to CharacterItem, so an inventory holding `{ entryId: \"sword-1\" }` resolved successfully and handed the caller an entry with no itemId, quantity or state — typed as a complete one. Matching an id proves an id matched and nothing about the rest of the object, so findInventoryEntry() now checks the SHAPE before the value leaves, and a fourth resolution issue, invalid-entry, keeps corrupt stored state distinct from an object that is gone: the first is a repair on the sheet and the second is a deletion, and reporting one as the other invites the wrong fix. Catalog membership stays outside that check, because whether an id is known depends on what a host has registered and a lookup that answered differently before and after a catalog load would be one nobody could reason about.",
    },
    "equipment.state.held-and-worn-are-not-one-bit": {
        id: "equipment.state.held-and-worn-are-not-one-bit",
        question:
            "`equipped: boolean` answered with one bit a question that has more than two answers. A sword in the hand and a breastplate on the chest are both equipped, but they are engaged by different parts of a body, become unavailable for different reasons, and are put on and taken off at different costs.",
        chosen:
            "A closed vocabulary of carried / held / worn, with `isEquippedItemState()` as the only place the equipped question is asked. Held and worn are distinct states that both satisfy an `equipped` Item requirement and both admit equippedEffects. An entry may be held or worn only at quantity exactly one; quantity zero is legal and means an emptied entry that contributes no effects and no requirement presence. Hands, body slots, conflicts, dual-wielding and readiness costs are deliberately NOT modelled.",
        rationale:
            "Hands, slots, Shu and weapon selection all need to know whether an object is in a hand, and while the field was a boolean each of them would have had to recover that from the Item definition — separately, and therefore eventually differently, which is the sensory-scope failure in a different domain. The predicate exists so the rule has one spelling: an inline `state !== \"carried\"` is the same test written a second way, and it is the spelling that changes meaning silently the day a fourth state is added, since a stowed or sheathed object would default to equipped, which is exactly backwards. The quantity-one rule is what makes a held entry an OBJECT rather than a pile: a stack of three swords in the hand names no particular sword, so nothing downstream could say which one Shu was applied to or which one broke — and refusing it here is what lets every later system assume the question is answerable. Stack splitting, the legitimate way a player reaches that state, is deferred rather than guessed at. Zero quantity is legal because an emptied quiver is still a quiver on the sheet, which is why possession is a positive count rather than the presence of a line; the alternative is deleting the container the moment it empties and inventing it again on the next arrow. What this ticket does NOT do is as load-bearing as what it does: there are no equip actions, `equipRequirements` are still not evaluated, `useEffects` are still not executed, and nothing decrements a quantity. The state records how an object is currently engaged; it does not claim to know what engaging it costs or what it excludes.",
    },
    "equipment.inventory.grouping-must-not-change-a-character": {
        id: "equipment.inventory.grouping-must-not-change-a-character",
        question:
            "Entries made a repeated `itemId` legal, and quantity above one was already legal, so the same objects had two spellings with two different meanings. getActiveItemEffects() contributes a definition's Effects once per ENTRY and cannot scale them by a count, so two Cursed Idols written as one entry of quantity two produced CHA -1 while two entries of one produced CHA -2. A character gained a modifier because a host had grouped identical objects differently.",
        chosen:
            "ItemDefinition declares a required `inventoryMode` of `individual` or `stackable`. An individual entry may hold zero or one; a stackable entry may hold any valid count but may declare no possessedEffects or equippedEffects, which findItemCatalogIssues() refuses over authored and custom definitions alike. Only a quantity-one entry may be held or worn, in either mode. useEffects are unaffected. Both authored Items are individual, and no stackable Item is authored.",
        rationale:
            "The tempting fix is to multiply passive Effects by the quantity, and it is wrong twice over: there is no multiplier in the Effect vocabulary — what would three copies of a grantTechnique or a modifyCheck even mean — and inventing one here would decide the whole of Phase 4's consumable semantics as a side effect of a validation bug. The other tempting fix is to pick a canonical grouping and normalise into it, which makes the engine silently rewrite a host's inventory and leaves the ambiguity live in every un-normalised copy. So neither representation is reinterpreted: one of them simply stops validating. That is why the mode is REQUIRED and never defaulted — a default would be applied silently to every Item an author had not thought about, and the two choices differ in what a sheet may legally say, which is not a question an omission should answer. The stackable-has-no-passive-Effects rule is the half that actually closes the hole, since without it a stack of fifty rations would apply its bonus exactly once and look correct. The division also matches how the objects behave in play rather than being a bookkeeping convenience: an individual Item is the kind of thing that gets held, worn, enchanted, broken and named, and a player who owns two of them expects to be able to point at one. Nothing stackable is authored, because consumables and ammunition are content this ticket did not design and an authored Item is a rule the moment it ships.",
    },
    "equipment.equip.requirements-are-a-moment": {
        id: "equipment.equip.requirements-are-a-moment",
        question:
            "An Item's equipRequirements gate wearing it. The obvious place to enforce them is character resolution, so a sheet is always consistent: an Item whose gate no longer holds would stop contributing, or revert to carried, and nobody could ever be wearing something they do not qualify for.",
        chosen:
            "The gate is asked at the TRANSITION and never again. resolveEquipmentTransition() evaluates equipRequirements when the destination is held or worn, and nothing re-checks them afterwards: an equipped Item stays equipped and keeps applying its equippedEffects when the requirement lapses, and only the next equip attempt is refused. Unequipping evaluates nothing at all. The destination alone decides which kind a transition is, so held -> worn and worn -> held are re-equips and are gated.",
        rationale:
            "Enforcing the gate during resolution makes resolveCharacter() a function that WRITES. A read that changes stored state is unsafe to run while a player edits, cannot be run twice with the same answer, and makes a character's equipment depend on the order facts were entered — lose the Trait for one round to a Condition and the armour falls off; regain it and the character is still undressed, because nothing put it back. The gentler version, suppressing the Effects while leaving the state, is worse rather than better: the sheet then says worn while the numbers say otherwise, and a player reading either one is misinformed. So the two questions are separated by TIME rather than by strength — may you put this on, asked once, versus does this work, which is a use requirement or an active contribution rule and belongs to a mechanic that does not exist yet. Unequipping is ungated for the same reason it would be cruel not to be: a character who no longer meets the requirement that let them don the armour must still be able to take it off, and a gate on the way out would trap them in it permanently. Reading the kind off the destination rather than the pair is what makes held -> worn correct: a rule phrased as \"are they becoming equipped\" answers no for a character who was already holding the thing, and would skip the gate on a transition that ends with the Item on their body. AMENDED once: the transition checked the inventory mode and the equip gate while findItemCatalogIssues() checked the inventory mode and the stackable-passive-Effects rule, so a stackable Item declaring equippedEffects was reported broken by the catalog and equipped perfectly happily a moment later, and a structurally malformed passive Effect was caught by neither. Two validators over one subject are two answers to one question, and the caller who asked the more permissive one never finds out — so the per-definition rules live in one findItemEquipmentDefinitionIssues() that both call, and the test asserts the PARITY rather than either side of it, since a test of the transition alone would pass again the next time a rule was added to the catalog only. Use content stays outside that validator deliberately: a potion whose healing Effect is malformed is a broken potion and not a reason to refuse to strap the belt it hangs from onto a character, and folding it in would let an unrelated authoring mistake block a transition that never reads it — which is also why the function is named for the EQUIPMENT surface rather than for the Item: a name claiming to validate the whole definition while deliberately skipping half of it is a name that will be trusted for the wrong thing, and the use path can add findItemUseDefinitionIssues() beside it. AMENDED AGAIN: the validator claimed host definitions may be anything and could not survive equippedEffects: [null], which reached the universal Effect validator and threw on the first read of `.type`; a non-array equippedEffects passed SILENTLY, because an object has no length and the validating loop ran zero times. Both vocabularies are hardened at the root rather than guarded at this one caller — a rule node's shape is checked before any field is read, an unrecognised discriminant is reported rather than falling off the end of a switch, a non-array list is refused, and isNonEmptyId takes unknown instead of promising a string the caller could not supply. The silent half was the more dangerous: a validator that throws is found the first time anyone runs it, while one that pronounces malformed content clean is found by a player whose Item does nothing. AMENDED A THIRD TIME: the transition handed resolved.requirementContext straight to the evaluator, so an equip gated on an attributeMinimum threw on requirementContext: {} — the same no-throw breach the Item use resolver had, still open on the other active Item operation. Both active Item operations now validate a caller-supplied requirement context through the one shared findRequirementContextIssues() before reading it and refuse what it reports as input_invalid; there is no second context validator. The transition validates at the moment the context would be read — every equip, re-equips included, whether or not the Item declares equipRequirements — and nowhere else. Unequipping reads no context and so never validates one: a gate on the way out would trap a character in their armour, and an unrelated context fault doing the same would be that trap arriving by another route. already-in-state and not-concrete-object are decided before the gate and keep their precedence, since neither reads the context. Item use differs in one deliberate respect: it validates the context as part of its input, before the reference is resolved, so its not-usable and quantity-unavailable outcomes are refused under a malformed context as well.",
    },
    "requirements.named.one-contract-three-domains": {
        id: "requirements.named.one-contract-three-domains",
        question:
            "Three domains had independently declared the same three fields. The Character action adapter had NamedRequirement { id, requirement, summary? }; a Skill's application had ApplicationRequirement with an identical body and a comment saying the shapes agreed ON PURPOSE, copied because nothing under character/ may import the adapter; and Item equip requirements were about to need a third.",
        chosen:
            "One declaration in the universal requirement vocabulary — rules/requirements.ts — with NamedRequirementResolution, resolveNamedRequirements() and namedRequirementDisposition() beside the evaluator. ApplicationRequirement and the adapter's NamedRequirement become ALIASES, the Skill layer's resolution and aggregate delegate rather than reimplement, and architecture.test.ts refuses a second interface body while permitting aliases. Ids must be non-empty and unique within their bundle, checked at catalog validation; acquisition requirements stay bare Requirement[].",
        rationale:
            "The comment explaining the copy was honest and the copy was still wrong. Two structurally identical declarations of one contract are two things that drift, TypeScript accepts assignment between them in both directions for exactly as long as they happen to match, and the failure surfaces as a runtime disagreement nobody's build flagged — the sensory-scope failure this codebase has already been bitten by once. The layering argument for the copy was also solvable rather than binding: the shared contract belongs BELOW all three, in the vocabulary every one of them already imports, so nothing has to reach for the adapter. Names are kept as aliases because \"an ApplicationRequirement\" reads better than \"a NamedRequirement on an application\" at a Skill call site and because both are public API. The id is supplied rather than derived, and that is the whole point of the shape: it is what a GM overrides by name and what a UI lists, so deriving it from the requirement's contents or its array position would silently break every stored override the moment content was rephrased or reordered. Uniqueness is scoped to the bundle rather than globally, because \"has-the-trait\" is a perfectly good id on two different Items and nothing ever holds two bundles' ids in one list. Acquisition requirements are deliberately left bare: they are asked once, when a character takes something up, and never need to be addressed again — giving them ids would be ceremony for a question nobody asks twice.",
    },
    "catalogs.registration.structure-now-existence-later": {
        id: "catalogs.registration.structure-now-existence-later",
        question:
            "A host could register any structurally-typed definition and the engine discovered the problem later, somewhere else. For Effects that \"later\" was not a diagnostic: resolveRuleEffects() ends its switch with a deliberate `never` exhaustiveness guard, so an Item registered with possessedEffects: [{ type: \"bogus\" }] made resolveCharacter() THROW for anyone carrying it — not return an EngineFailure, throw — addressed to whoever resolved a character rather than to whoever wrote the content.",
        chosen:
            "A registration barrier. createRegistry() takes a REQUIRED structural validator and runs it before anything is stored: a malformed definition is refused, nothing is written, and a refused replacement leaves the previous definition byte-identical. The `never` guard in resolution is kept exactly as it is. Structure is judged at registration and EXISTENCE is not — cross-catalog reference checks stay in findCatalogReferenceIssues(), after every catalog has loaded. The rule-bearing field walk moved from catalogs.ts down into rules/definitions.ts so both the barrier and the reference checker read one list, and the universal Effect, Requirement, named-requirement and combined validators now take `unknown` and narrow through type predicates with no assertion into the rule vocabulary at all.",
        rationale:
            "Softening the exhaustiveness guard was the obvious fix and the wrong one: it was added after ten Body effect variants were introduced and silently dropped, so turning it into a skip would trade a loud developer error for exactly that silence, to paper over a problem that is not resolution's. The barrier makes the guard unreachable from host data instead, which preserves both properties — a developer adding an unhandled variant still fails loudly, and a host offering nonsense gets a refusal it can read. Validating BEFORE the write is what makes a refusal atomic, and it is the half that is easy to get wrong: re-registering an existing custom id is an EDIT, so a write followed by a failing check would let someone correcting a typo lose the entry they were correcting and end up with neither version. The validator is required rather than optional because an optional structural check is one every new registry starts life without, and the failure is silent; a domain with nothing to check passes a named `declaresNoRules` so that \"this domain has no rules\" is a claim someone made rather than an empty function that reads as an oversight. Existence deliberately stays out: content refers forward — a Clan may grant a Technique registered a moment later — and refusing that at registration would make load order a rule nobody authored, so a host loading its catalog alphabetically would see failures another host would not. The walk had to MOVE rather than be copied, because catalogs.ts imports every registry and a registry importing catalogs.ts would close a module-initialisation cycle that TypeScript resolves happily and Node does not. And the validators take `unknown` because that is what a host's definition is: `candidate as Effect` asserts precisely the thing being tested, which makes every field read after it well-typed and unfounded — the day a new field is read without a guard, nothing complains. COMPLETED afterwards: the first pass checked common identity and the universal rule vocabulary, which left every domain's own structure outside — a Skill with no application, an Injury with an impossible recovery ceiling, a Reference Form whose parts attached to nothing all registered cleanly and were reported by catalog validation later, which is the situation the barrier existed to remove. Each domain now composes its local validator with the universal walk, the three Body domains that had honestly declared no RULES got real SHAPE validators (a BodyPart with a negative Volume carries no rules and is still unresolvable), and declaresNoRules survives as an escape hatch nothing uses, with an architecture rule that fails if anything reaches for it again. The split WITHIN a domain follows the same line as the split between registration and catalog: self-parenthood is a fact about one definition and is refused at the door, while descent from an unknown Species and a circular parent chain need the catalog and stay post-load. Description joined id and name as a universal check ahead of the domain rules, so a definition wrong in two ways reports the one that has to be fixed first. The cost was seven more existing tests that registered malformed content to reach a downstream check; each now asserts the refusal and, where the downstream rule still matters, asks it of the definition directly — Recovery is HANDED its Injury definitions rather than reaching for a catalog, so that check is still reachable and still tested. COMPLETED AGAIN after review found two gaps inside the new validators. The first was a boundary the composition had merely MOVED: several domain validators cast hostile values into older typed ones, so mastery: null, application: null, a non-array stages and a missing recovery each reached a function that read a field off them and threw — from inside the validator whose job was to complain about that exact fault. The small ones now take unknown and narrow; findSkillApplicationIssues stays typed and is preceded by field guards, because it is large and deeply recursive and widening every field it reaches would be a rewrite of the contract layer for a fault one block removes. The guard list is deliberately SHORT: a missing Range already produces \"must state its Range\", which is a better sentence than any shape guard would write, so only the fields that actually dereference unchecked are covered, and a sweep over every domain-owned compound field is what keeps that true when a field is added. The second gap was topology: \"exactly one root\" is not acyclicity, and a form with a good root beside two slots parented to each other has one root, no dangling parent reference, and a disconnected cycle. Every slot now walks its parent chain home. BodyParts also gained the two invariants the older validator had and the new one had not: a repeated tag, which is membership written twice, and a heightAxisSign that must be exactly 1 or -1 — a 0 there is a direction pointing nowhere and any magnitude is a second silent scaling of a contribution that already has one. FINISHED after a third review found the quieter half of the same class: four fields that were ACCEPTED rather than throwing. findMasteryTrackIssues filtered its rank list down to the entries that were objects, which is what a reference COLLECTOR should do — a collector runs beside a validator and would otherwise report one fault twice — and which in a VALIDATOR turned ranks: [null] into a track with no ranks that registered perfectly cleanly. An absent treatmentRequired was read as false, so recovery: {} became the real mechanical claim that an Injury heals on its own; the field is required, so an author who omitted it has not chosen the permissive answer, they have not answered. An applicability was counted as declared because the field EXISTED, so { bodyParts: 42 } satisfied \"must declare anatomical applicability\" while naming nothing a body could have — the selector vocabulary owns what a valid one is and is now asked rather than second-guessed. And the Reference Form validator had dropped two invariants the older code had: a duplicate continuityKey, which makes ambiguous the one identity an Injury and a transformation both match on, and a blank attachment site, which prints as nothing and matches nothing and is indistinguishable from having declared none except that the author believed otherwise. The test change matters as much: the sweep had asserted that storage agreed with the verdict, which reads like coverage and cannot fail — register() returns the verdict it just acted on — and an aggregate \"most were refused\" threshold was the same mistake at a distance, staying green while any one required field was wrongly accepted. Both are replaced by every required compound field named with the values it must refuse, one at a time, plus a control proving the rule does not pass by refusing everything. FINALLY, one nesting level deeper: guarding an Injury's applicability.bodyParts and an Anatomical Point's placement.selector as OBJECTS was necessary and not sufficient, because validateBodyPartSelector() then measured and iterated every filter inside them on the strength of a caller's cast — so { ids: 42 } is an object and threw on .length, and { states: {} } threw on iteration, from inside the function whose whole job is to say a selector is wrong. Hardened at the selector rather than at either caller, because two guards would be two copies of one rule and the copies are what drift, and swept through BOTH paths, because a fix at a shared boundary is only worth anything if every route to it is covered. A filter that is not a list reports malformed-filter rather than empty-<kind>-filter, since the two have different fixes: an empty list needs an entry added and a number needs the field replaced, and reporting them identically sends an author to do the first when they need the second. AND FINALLY, the selector's SEMANTICS rather than its shape: three selectors validated cleanly and were then reinterpreted by matching, which is worse than being refused because the content is accepted, runs, and means something the author did not write. { all: true, ids: [...] } selects everything and the ids are dead text; a tagMode outside the vocabulary falls past matchesBodyPartSelector's `=== \"all\"` branch and silently loosens \"has all these tags\" into \"has any of them\"; and a state outside BODY_PART_STATES matches nothing a body can be, so a filter that reads as narrow selects nothing. The type had declared all three constraints already — AllBodyPartsSelector marks every filter `never`, FilteredBodyPartSelector declares `all?: false` and a closed tagMode — and none of them reached host content, which never goes through the type. Each now has its own diagnostic rather than a shared invalid-selector, because the fixes differ: remove the filters, correct the flag, choose a real mode, name a real state. The filter-field list is written once and both the exclusivity check and the presence check read it, so a field added to the selector and not to that list cannot be silently permitted beside `all`.",
    },
    "equipment.use.an-activation-is-an-event": {
        id: "equipment.use.an-activation-is-an-event",
        question:
            "An Item declared useEffects and useRequirements from the start, and nothing executed either. Using one has to settle three things the passive timings never did: whether a use gate is a moment like the equip gate or a condition asked every time; what executing an Effect means when the vocabulary describes contributions to derived state rather than writes to stored facts; and when a unit is used up — which the Rulebook never states, and which an inventory mode, a quantity or a name would each seem to answer.",
        chosen:
            "resolveItemUse() is one pure resolution over an InventoryItemRef. It validates the use surface with the same rules registration applies, treats an Item as usable only when it declares a use Effect, a use gate or consumesOnUse: true, requires a positive quantity, evaluates NAMED useRequirements against the pre-use RequirementContext, and then resolves useEffects exactly once through resolveRuleEffects() with the definition and entry ids as provenance. A unit is consumed only when consumesOnUse is true: exactly one, from the referenced entry, keeping an emptied entry on the sheet and returning a held or worn one to carried in the same replacement Character. Refusals are dispositions carrying no Effects and no Character, and their ItemUse reports consumed: 0. Malformed questions are EngineFailures. No action intent, cost, target or durable mutation is produced.",
        rationale:
            "useEffects are events, and collecting them anywhere passive is how a potion heals continuously for being in a bag — so they reach the use result and nothing else, once per activation, because a use scaled by stack size would make grouping change what happened, which is the failure inventoryMode exists to prevent. Resolving them into the canonical sourced output rather than applying them is deliberate: the facts they would change — Attributes, BP, Body Strength — have owning mechanics, and a use resolver that rewrote them would be a second implementation of every one; routing consequences belongs to the action-intent and settlement work that follows. Consumption is declared because every inference is wrong for someone: stackable is not consumable (a whetstone survives a sharpening), having use Effects is not consumable (a wand), and a guess right for potions fails silently for the first reusable thing an author makes stackable. A refusal reports what happened rather than what would have, because a caller that applied a proposal read off a refusal would decrement a stack for a use that never occurred. The use gate is named for the equip gate's reason — it is refused to a player by name and overridden by id — but asked every time, because it is precisely the keep-requiring-something-to-function mechanic the equip gate was careful not to be. The equipment and use surfaces are validated apart in both directions, so a malformed useEffect cannot block an unequip, and meet at registration built from the same rules, so the barrier and the resolver cannot disagree about a definition. AMENDED once: the resolver checked only that requirementContext was an object, so a resolved character carrying requirementContext: {} passed the guard and then threw inside the evaluator the moment an attributeMinimum gate read `attributes.base` — the malformed-input-becomes-exception the resolver's contract rules out. The repair is at the shared boundary rather than in this caller: findRequirementContextIssues() in rules/resolution.ts takes unknown and checks every field the evaluator dereferences — the three Attribute layers and every score, level, the optional id lists, the Mastery records, both Item lists, and incomplete against the closed collection vocabulary — and the use resolver refuses anything it reports as input_invalid, before any requirement is read and regardless of whether the Item has a gate. It checks that a context can be READ and gives meaningful answers — finite numbers, string ids, real collections, since a bare string id list would answer hasTrait by substring — and deliberately not Level or Mastery legality, which progression and capabilities own from above the rules layer; importing their rules downward would invert the dependency the universal vocabulary exists to keep. The evaluator itself stays typed and unguarded, because a context is validated once where it crosses a boundary rather than on every requirement read.",
    },
    "rules.bundles.one-representation-per-requirement-list": {
        id: "rules.bundles.one-representation-per-requirement-list",
        question:
            "The rule-bundle walk carried a named gate twice: once as bare Requirement trees projected out of the named entries, for reference checking, and again in a second field beside it in named form, for id and summary checking. Making Item use requirements named would have put a third gate into that arrangement.",
        chosen:
            "A RuleBundle holds one RuleRequirementBundle, discriminated as bare or named, containing the list exactly as authored. Acquisition, Mastery and stage requirements are bare; Skill application, Item equip and Item use requirements are named. findRuleBundleIssues() validates a named list as a named list, ruleBundleRequirementTrees() is the only projection to inner requirements and is used by the reference collector alone, and architecture.test.ts refuses the parallel field's return. The walk recognises an Item by any rule-bearing field, gates included.",
        rationale:
            "Two fields describing one list are two things a caller can disagree about, and the projection was the less truthful of the pair: it had already discarded every entry it could not read, so a malformed named list and an empty one were indistinguishable through it, and a nested fault was reported twice under two paths. Validating the wrapper directly reports each fault once, at the path the author wrote, and confining the projection to a collector — which may skip what it cannot read because a validator beside it reports the fault — keeps the lossy view in the one job where losing things is correct. Recognising an Item by its gates as well as its Effect lists closes a gap the change exposed: an Item declaring only a use gate fell through to the generic walk, which looks for a top-level requirements field, and had its references checked by nothing.",
    },
