...
**Rule validators take `unknown`, narrow through type predicates, and do not throw.** No `as Effect` anywhere: an assertion tells the compiler the value *is* valid content, which is the claim these functions exist to test, so every field read after it would be well-typed and unfounded. `findEffectValidationIssues()` and the requirement validator each guard the node's shape before reading any field, report an unrecognised discriminant as `unknown-rule-type` rather than passing it silently, and refuse a non-array `effects`/`requirements` as `malformed-rule-node` rather than looping zero times over a missing `length`. Both used to throw on `null`, on `{ type: "modifyCheck" }` with no `check`, on `{ type: "all" }` with no children, and on any id field that was absent — `isNonEmptyId` took `string` and called `.trim()`, a promise the caller could not keep for host JSON.

**The registration barrier.** `createRegistry()` takes a **required** structural validator and runs it **before anything is stored**, so a malformed definition never enters a catalog. A forgotten validator is a compile error; a domain with no rules to check passes the named `declaresNoRules` rather than an anonymous `() => []`, which `architecture.test.ts` refuses.

- **Refused means nothing stored**, and a refused *replacement* leaves the previous definition byte-identical — re-registering an id is an edit, so validating after the write would let a host correcting a typo lose the entry it was correcting.
- **Structure only, never existence.** Content refers forward: a Trait may grant a Technique registered a moment later. Cross-catalog reference checks stay in `findCatalogReferenceIssues()`, after every catalog has loaded, so load order is not a rule.
- **Every domain's local structure runs too**, composed with the universal walk via `composeStructuralValidators()`: Skill applications and Mastery tracks, Technique Mastery, Condition stages, Injury applicability and recovery, Species/Trait self-parenthood, Item mode and both surfaces, BodyPart measurements, Reference Form topology, Anatomical Point categories. `declaresNoRules` still exists and nothing uses it — the three Body domains have real shape validators now, and an architecture rule fails if anything reaches for the escape hatch again.
- The split *within* a domain follows the same line: "is its own parent" is a fact about one definition and is refused at registration; "descends from an unknown Species" and "has a circular parent chain" need the catalog and stay post-load.
- Universal identity is settled **before** domain rules run — id, name, then description — so a definition with no usable id produces one actionable complaint rather than a list nobody can address.
- The older validators the domain ones delegate to (`findMasteryTrackIssues`, `findStageTrackIssues`, the Injury applicability/recovery pair) take `unknown` and narrow. `findSkillApplicationIssues` is the exception and stays typed — it is large and deeply recursive, so it is preceded by field guards covering exactly what it dereferences unchecked. A hostile sweep over every domain-owned compound field is what keeps that list honest.
- `validateBodyPartSelector()` takes `unknown` and guards every filter before measuring or iterating it. It is a **shared** boundary — an Injury's `applicability.bodyParts` and an Anatomical Point's `placement.selector` both reach it from registration — so it is hardened there rather than at either caller, since two guards would be two copies of one rule. A filter that is not a list reports `malformed-filter`, distinct from `empty-*-filter`: `ids: []` needs an id added, `ids: 42` needs the field replaced.
- It also enforces the selector's **semantics**, not just its shape: `all: true` may carry no filter or `tagMode` (`all-with-filters`), an `all` on a filtered selector must be exactly `false` (`invalid-all`), `tagMode` must be `"all"` or `"any"` (`invalid-tag-mode`), and every state must be one of `BODY_PART_STATES` (`invalid-state`). All four used to validate and then be *reinterpreted* by matching — `{ all: true, ids: [...] }` selects everything with the ids as dead text, a bogus `tagMode` falls past the `=== "all"` branch and loosens "all these tags" into "any of them", and a bogus state matches nothing a body can be.
...
`application` is **required**. A Skill is by definition a concrete thing a character does on purpose, so one without a contract is an unfinished definition, not a kind of capability. The engine still never *invents* one: `minimalSkillApplication()` is authoring scaffolding (a real, visible, free-adjudication contract) that an authoring tool writes into content — `architecture.test.ts` holds that nothing which resolves, validates or authors ever calls it, and host-registered JSON with no application is refused and reported by `findSkillCatalogIssues()`.
...
**Mastery changes** are typed operations over a closed field list (`structuredActionCost`, `executionDuration`, `rangeMinimumMetres`, `rangeMaximumMetres`, `maximumTargets`, `exertionLoad`, `baseAuraCost`, `requiredOutput`) plus `replace` of `threatens` / `travel` / `role`, `permit` / `prohibit` of a timing / focus kind / target kind, and — because Mastery is *depth* — `field: "outcomeOutput"` addressing one branch's own output by `(outcomeId, outputId)` to move its magnitude. Deliberately not a deep partial. Catalog validation rejects a trackless Skill declaring changes, out-of-range or duplicate thresholds, a change to a field the base does not declare, an outcome/output the application does not declare, an output carrying no authored `amount` to move, and any rank whose projection would be an invalid profile.
...
`buildSkillActionProfile(resolved, context?)` takes `SkillApplicationContextValues` — each entry carrying the `profileId` it answers *and* the value — and **re-runs the structural validation first**, because catalog validation is not on the projection path: a host that registers a Skill with no Range, reads the complaint and projects anyway would otherwise get a profile with no distance restriction. Only an explicit `{kind:"none"}` becomes an absent `ActionProfile.range`; an omitted one is never translated into it. It then fails when a context-derived field is unsupplied, answers a different profile, is supplied for a field authored as fixed (refused, not ignored), or fails its neutral-domain validator.

Both halves of that boundary are swept with hostile values — the *specification* arriving from host content and the *supplied value* arriving from the caller, which is the wider boundary since a context value is computed per attempt. All missing fields are reported at once. **The finished `ActionProfile` holds only resolved metres and milliseconds**; no contextual specification reaches `actions/` or `spatial/`, and `architecture.test.ts` asserts `DistanceInterval` still has exactly `kind` / `minimumMetres` / `maximumMetres` and that `spatial/` names no Skill, Body, Aura or Reaction concept.
...
> ⚠️ **Constraint on the execution ticket.** The projection is **lossy, necessarily and permanently**. `ActionProfile` carries one check scope; an application carries the opponent's scope, `tiesFavor`, a fixed check's `tiePolicy`, the exertion and Aura costs, and the outcome branches — none of which fit in a profile and none of which should. **The projected `ActionProfile` is not the whole executable contract.** Preparation must be handed the `EffectiveSkillApplication` alongside it; an execution layer that takes the profile alone will silently resolve every opposed Skill as unopposed and charge nothing for any of them.
...
**The context is validated where the gate reads it, and only there.** Every equip — `held` or `worn`, re-equips included, whether or not the Item declares `equipRequirements` — passes `resolved.requirementContext` through the shared `findRequirementContextIssues()` immediately before the gate is evaluated; a context it cannot read is `equipment.transition.input_invalid`, never a throw from inside the evaluator. An unequip reads no context and so never validates one: an unrelated context fault must not trap an Item in its equipped state. `already-in-state` and `not-concrete-object` are decided before the gate and keep their precedence over a malformed context.
...
```
equipment.transition.{input_invalid,destination_invalid,reference_invalid,
                      character_mismatch,entry_unknown,entry_invalid,
                      item_unknown,definition_invalid}
```
...
- **The requirement context is validated before it is read.** `findRequirementContextIssues()` / `isRequirementContext()` in `rules/resolution.ts` take `unknown` and check every field the evaluator dereferences — `attributes.{stored,base,resolved}` and each score, `level`, the optional id lists, `skillMastery`/`techniqueMastery`, `items.{possessed,equipped}`, and `incomplete` against `REQUIREMENT_COLLECTIONS`. A context that fails is `equipment.use.input_invalid`, never a throw from inside the evaluator. Both active Item operations ask this one validator before reading a context; they differ in *when*. Item use validates the context as part of its input, before the reference is resolved, so its `not-usable` and `quantity-unavailable` outcomes are refused under a malformed context too. The equip transition validates only on the equip path, so unequipping and the outcomes decided before its gate stay independent of the context. It checks readability, not legality: Level 1–30 and Mastery bounds are owned above `rules/`.
...
The Skill definition is then validated with the same rules registration applies (`findContentStructuralIssues` + `findSkillDefinitionStructuralIssues`) **before** its application is read, and the application is passed to `resolveSkillApplication()` explicitly. Identity is not soundness: `{ id: "measured-art", application: 42 }` cleared the id check, had its nonsense application refused downstream, collected nothing, and returned an ordinary empty *success* — a broken catalog reading as a character with no bonuses. An `application: null` was worse: `resolveSkillApplication()` resolves `input.definition ?? getSkillDefinition(skillId)?.application`, so it fell back to the engine's own registry and took availability from the global catalog while reading rules from the caller's. Validating first makes the application guaranteed present, which is what lets the conditional spread — and the fallback it was hiding — go.
