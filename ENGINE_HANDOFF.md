...
**Rule validators take `unknown` and do not throw.** `findEffectValidationIssues()` and the requirement validator each guard the node's shape before reading any field, report an unrecognised discriminant as `unknown-rule-type` rather than passing it silently, and refuse a non-array `effects`/`requirements` as `malformed-rule-node` rather than looping zero times over a missing `length`. Both used to throw on `null`, on `{ type: "modifyCheck" }` with no `check`, on `{ type: "all" }` with no children, and on any id field that was absent — `isNonEmptyId` took `string` and called `.trim()`, a promise the caller could not keep for host JSON.

⚠️ **Effect *resolution* still throws on an unrecognised discriminant.** `rules/resolution.ts` ends its switch with a deliberate `never` exhaustiveness guard, which exists to catch a developer adding an Effect variant nobody handles — and host data can currently reach it, so registering an Item with `possessedEffects: [{ type: "bogus" }]` makes `resolveCharacter()` throw for anyone carrying it. Validation now catches this before it gets there, but resolution has no such contract of its own. Deciding what resolution should do with unrecognised content is a design question, not a hardening pass.
