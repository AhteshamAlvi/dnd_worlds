...
The [[Conditions Vows and Risk (Rules)|sincerity audit]] promises teeth. Keep an actual list of every PC vow with three columns: *clause · what breaking looks like · when I last tested it.* Test each about once an arc — not as gotcha, as spotlight: the vow-keeping scene is that player's best scene. If a vow goes two arcs untested, it's cosmetic in practice; tell the player its multiplier is fading toward ×1 and let them renegotiate (with fiction: their own depths noticing the oath got easy).

## Power disparity at the table

The math permits parties to meet things they cannot fight ([[Scale Speed and Magnitude]]). Running that honestly:

- **Telegraph magnitude in fiction before contact:** the Watch captain's face when the breach class comes in; bounty postings with band stamps; the Gillian's footprint before the Gillian ([[NPCs Creatures and Encounters]] for band tells). Players should almost always know they're outclassed *before* initiative.
- **Unwinnable ≠ unplayable.** Stakes shift from "defeat it" to: escape route, civilians, delay for evacuation, the one telegraph it has, negotiation if it thinks ([[Supernatural Races]]: Awakened Beasts, Adjuchas+). Some of the best sessions are weather management.
- **Never fudge magnitude to save a fight you mis-telegraphed.** Apologize, rewind the scene entry, re-telegraph. Fudging Soak once teaches players that numbers are mood.
- **Mixed-band parties** (it happens): aim encounters at the middle band; give the top-band PC a *second* job in the scene (protect, contain, duel the enemy elite) so their magnitude doesn't delete the fight the rest are having.

## Consequences and collateral

Elite combat wrecks terrain by rule (P vs object Soak, [[Scale Speed and Magnitude]]). Track it: property has owners, cities have codes, the Registry has forms ([[Society Law and Licenses]]). Collateral is the tax that keeps high-band PCs *social* creatures. A master who fights carelessly downtown answers to the same licensing regime that made them respectable.
...
Rule at the table for tonight; log it; settle it between sessions against this book; if the book is silent, the [[Core Resolution]] adjudication order decides; if it's *still* ambiguous, the player-favorable reading wins once and the GM's considered ruling wins thereafter. Write the ruling into the campaign's house-rules note — this Rulebook is a repository; grow it.
...
Before session: affinity/variation for any PC approaching divination; In-contest results for planted ambushes; the sincerity audit for any vow proposed mid-session last time. A prepared secret roll is a poker face you don't have to perform.
