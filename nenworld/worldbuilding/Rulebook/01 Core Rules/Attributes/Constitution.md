[[0 Index|Index]] › [[0 Rulebook|Rulebook]] › Core Rules › [[0 Attributes|Attributes]]

**Constitution is capacity — how much aura you hold and how long your body can push.** For a Nen user it is the single defining stat: **your maximum Aura Pool is derived directly from CON, and your CON score doubles as your power band.**

## What CON governs

- **Maximum Aura Pool** — a derived stat, computed from CON exactly as Body HP is computed from [[Vitality|VIT]], with **no cap** ([[Aura Statistics]]):
  > **Max Aura Pool = round( 30 × 1.6^(CON − 10) )**
  Each point of CON multiplies the pool by ~1.6 (**+3 CON ≈ ×4, +6 CON ≈ ×16**). You do not accumulate aura points toward a ceiling — you raise CON, and the pool follows.
- **Stamina, fatigue, and sustained exertion** — forced marches, holding your breath, working past exhaustion.
- **Resistance to poison, disease, and environment** — the **Fortitude (CON)** save (as distinct from Fortitude (VIT), which resists bodily *damage*) ([[Attributes and Skills]]).
- **Endurance** skill (CON): forced march, breath-holding, resisting exhaustion, and **Concentration under fire** — holding a sustained principle while taking Body damage is a **CON (Endurance)** check, DC 10 + damage÷10 ([[Combat Core]], [[Nen Principles (Rules)]]). [[Ten]] III+ holds itself.
- **Aura-training checks** run on CON (Endurance): the awakening **Meditation** program, learning [[Ten]] and [[Zetsu]], and sustaining [[En]] past (CON mod + 2) minutes ([[Awakening and the Path]], [[Nen Principles (Rules)]]).
- **Suffocation** lasts CON *score* in rounds of effort before Dying; **extreme cold/heat** is a CON save ([[Injury Recovery and Conditions]]).

## Growing CON — the aura engine

Because the pool is derived from CON, **you grow your reserves by raising CON** ([[Nen Growth]]). Sustained Nen aura-tempering (the daily [[Ten]]/[[Ren]] discipline) is the regimen that does it, and **CON is the one attribute that Nen-tempering permanently drives past the mundane human cap of 20 into the superhuman band (21–30)** — double time-cost per point past 20, but no wall. This is the mechanical reason trained Nen users become physically superhuman, and why elite rarity is arithmetic (a dozen breakthroughs to Master), not a ceiling.

## CON → Aura Pool (every awakened-relevant value)

| CON | Max AP | | CON | Max AP | | CON | Max AP |
|---|---|---|---|---|---|---|---|
| 8 | 12 | | 16 | 503 | | 24 | 22k |
| 9 | 19 | | 17 | 805 | | 25 | 35k |
| 10 | 30 | | 18 | 1.3k | | 26 | 55k |
| 11 | 48 | | 19 | 2.1k | | 27 | 89k |
| 12 | 77 | | 20 | 3.3k | | 28 | 142k |
| 13 | 123 | | 21 | 5.3k | | 29 | 227k |
| 14 | 197 | | 22 | 8.4k | | 30 | 363k |
| 15 | 315 | | 23 | 14k | | — | — |

*(A pool is only usable once awakened; a fresh awakener's weakness comes from low [[Ren]] output and poor Control, not from a small pool — [[Aura Statistics]].)*

## The Constitution scale (1–30)

| Score | Mod | Stamina / body | Awakened pool |
|---|---|---|---|
| 1 | −5 | Faints at the slightest effort; chronically ill. | ~0 |
| 2 | −4 | Bedbound-frail; needs help to stand. | ~1 |
| 3 | −4 | Still very frail, but can cross a room unaided on a good day. | ~1 |
| 4 | −3 | Very low stamina; sickly. | ~2 |
| 5 | −3 | A little steadier, but still wears out fast. | ~3 |
| 6 | −2 | Tires fast; poor resistance to illness. | ~5 |
| 7 | −2 | Slightly hardier, but still catches every passing bug. | ~7 |
| 8 | −1 | Somewhat frail; a light day tires them. | ~12 |
| 9 | −1 | Mildly under-conditioned; gets through an ordinary day. | ~19 |
| 10 | +0 | Average adult; a full day's labour tires them. | 30 |
| 11 | +0 | A touch above average; handles a long day without much complaint. | 48 |
| 12 | +1 | Hardy; rarely sick. | 77 |
| 13 | +1 | Solidly hardy; long days don't slow them. | 123 |
| 14 | +2 | Trained endurance; marches well. | 197 |
| 15 | +2 | Seasoned endurance; recovers fast between exertions. | 315 |
| 16 | +3 | Exceptional constitution; tireless. | 503 |
| 17 | +3 | Elite constitution; a true professional's stamina. | 805 |
| 18 | +4 | Peak human — elite endurance athlete. | 1.3k |
| 19 | +4 | World-class endurance; borders on unbelievable. | 2.1k |
| 20 | +5 | The human limit of endurance and vigor. | 3.3k |
| 21 | +5 | Just past the human limit of endurance — the threshold Nen-tempered body. Only sustained aura-tempering, a racial gift, or a Mutation reaches here. | 5.3k |
| 22 | +6 | **Superhuman (Nen-tempered):** shrugs off exertion that would fell an athlete. | 8.4k |
| 23 | +6 | The professional Nen body; holds a principle through injuries that would drop most fighters. | 14k |
| 24 | +7 | The elite Nen body. | 22k |
| 25 | +7 | Master-tier constitution; fights at full output long after others collapse. | 35k |
| 26 | +8 | Days of combat without flagging. | 55k |
| 27 | +8 | An ocean of aura beginning to show; grandmaster-tier vigor. | 89k |
| 28 | +9 | Grandmaster/calamity vigor. | 142k |
| 29 | +9 | An ocean of aura; near-unkillable stamina. | 227k |
| 30 | +10 | Apex — inexhaustible, [[Meruem]]-tier reserves. | 363k |

*Only CON reaches the superhuman rows through training; every other attribute needs race, [[0 Mutants|Mutation]], or temporary [[Nen Principles (Rules)|Chū]].*
