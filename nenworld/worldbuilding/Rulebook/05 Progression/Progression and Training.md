...
| *Session cap* | **5** |
| Arc completed | +2 |
| A living rival active this arc | +1/arc |

## Spending TP

All costs; fiction gates in the right column. Time can run concurrently with adventuring for **standard**-intensity training; **dedicated** requirements need real downtime.

| Purchase | TP | Fiction gate |
|---|---|---|
| Skill/attack/save proficiency: Trained | 2 | Practice in play or 2 weeks |
...
A program = **focus + duration + weekly check + success threshold** ([[Awakening and the Path]] shows the format for the Principles). Defaults:

- **Weekly check:** the program's listed attribute/skill; DC 13 baseline (DC 10 with a competent teacher, DC 16 self-taught novel material).
- **Successes needed:** rank I: 2–3 · II: 3 · III: 4 · IV: 5 · V: 6, spread over the base time in [[Nen Growth]]. Weeks that fail count time, not progress.
- **Partial success:** hitting half the successes when time runs out banks them — resume later without starting over.
- **Nat 1 on a self-taught week:** 1 Strain ([[Injury Recovery and Conditions]]) — you pushed something wrong.
- **Fatigue:** more than 12 consecutive dedicated weeks without a rest week: 1 Strain per further week. Bodies and spirits need slack; masters schedule it.
...
For skips (weeks to years): set the calendar, apply [[Nen Growth]] rates honestly, cap by the throughput limit per *narrated block* (a 1-year skip ≈ 4 downtime blocks). Roll one representative program check per block (player's choice where the drama was). Print the before/after sheet — the visible jump is the reward.

## Temporary transcendence

The moments the fiction runs on — outlined here, priced elsewhere:
